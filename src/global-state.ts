...
async function getMarkdownFilesFromFs(dir: string) {
  ...
  const entries = await git.walk({
    ...
    map: async (filepath, [entry]) => {
      ...
      return [filepath, new TextDecoder('utf-8').decode(content)]
    },
  })
